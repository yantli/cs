...
def save_whole_file(original_file_path):
    ...
    with open(original_file_path, 'r', encoding = 'utf-8') as f:
        ...
        for row in filereader:
            ...
            cs_lines = []
            n = 0
            while n < len(whole_file)-1:
                cs_lines.append(all_lines[n])
                n += 2
    return all_lines, cs_lines
...
def trans_vector_finder(original_file_path, zh_word_vec_path):
    ...
    for i in range(len(trans_words)):
        simp_tradi_dict[trans_words[i]] = traditional_trans_words[i]
    
    trans_vec_dict = {}
    with open(zh_word_vec_path,'r', encoding='utf-8') as vecfile:
        for line in vecfile:
            if len(line.split()) > 0 and line.split()[0] in trans_words:
                word = line.split()[0]
                if word not in trans_vec_dict.keys():
                    trans_vec_dict[word] = [line]
                else:
                    trans_vec_dict[word].append(line)

            # if the vector is for a traditional word but its simplified version is not yet in the dict:        
            elif len(line.split()) > 0 and line.split()[0] in traditional_trans_words and chinese_converter.to_simplified(line.split()[0]) not in trans_vec_dict.keys():
                word = line.split()[0]
                if word not in trans_vec_dict.keys():
                    trans_vec_dict[word] = [line]
                else:
        ...
        # The trans_vec_dict may have words with both forms. We want to keep the simplified key only.        
        uniq_trans_vec_dict = {}
        duplicated = []
        traditional_vector = []
        for simp, tradi in simp_tradi_dict.items():
            if simp in trans_vec_dict.keys() and tradi in trans_vec_dict.keys() and simp != tradi:
                duplicated.append(tradi)
        for key, value in trans_vec_dict.items():
            if key not in duplicated:
                ...
                if word in trans_words and word not in uniq_trans_vec_dict.keys():
                    uniq_trans_vec_dict[word] = numbers
                elif word in traditional_trans_words and word not in uniq_trans_vec_dict.keys():
                    uniq_trans_vec_dict[chinese_converter.to_simplified(word)] = numbers
                    traditional_vector.append(word)

    return uniq_trans_vec_dict
...
def calculate_cosine_similarity(eng, trans):
    cosine_similarity = np.dot(eng, trans) / (np.linalg.norm(eng) * np.linalg.norm(trans))
...
def gather_cos_similarity(original_file_path, eng_word_vec_path, zh_word_vec_path):
    ...
    for line in cs_lines:
        ...
        trans_word = line[8]
        sent_id = line[2]
        if eng_word in eng_vec_dict.keys() and trans_word in trans_vec_dict.keys():
            eng_wordvec = eng_vec_dict.get(eng_word)
            trans_wordvec = trans_vec_dict.get(trans_word)
